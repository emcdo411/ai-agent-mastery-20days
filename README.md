...
flowchart LR
  ...
  subgraph FEATURE["Feature Engineering"]
    B1["Clean and impute"]
    B2["Deduplicate and outlier rules"]
    B3["Feature store"]
  end

  subgraph TRAIN["Modeling and Evaluation"]
    C1["Experiment tracker"]
    C2["Train candidate models"]
    C3["Cross validate and compare"]
    C4{"Passes metrics and fairness?"}
  ...
  subgraph DEPLOY["Deployment Targets"]
    ...
    E2["Docker image to K8s"]
    E3["Secrets store"]
  end

  subgraph APP["Analytics and BI"]
    F1["Streamlit app"]
  ...
  subgraph MLOPS["Monitoring and Guardrails"]
    G1["Data quality and drift monitor"]
    G2["Model performance and latency"]
    G3["Bias and fairness monitor"]
    G4{"Drift or degradation?"}
  ...
  %% ==== MAIN FLOW ====
  A1 --> A2 --> A3
  A3 -->|No: reject and log| G6
  A3 -->|Yes| B1
  B1 --> B2 --> B3 --> C1
  C1 --> C2 --> C3 --> C4
  C4 -->|No: tune or search| C2
  C4 -->|Yes| C5
  C5 --> D1
  C5 --> D2
  D1 --> D3
  ...
  %% ==== OPTIONAL STYLING (safe colors) ====
  classDef decision fill:#2A2F45,stroke:#FFD166,color:#FFF5CC,stroke-width:1.2px;
  classDef stage fill:#1E2230,stroke:#8FA2FF,color:#E6E8FF,stroke-width:1.0px;
  class A3,C4,D3,G4 decision
  class INGESTION,FEATURE,TRAIN,SERVE,DEPLOY,APP,MLOPS stage
